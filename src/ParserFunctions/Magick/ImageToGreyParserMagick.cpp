 ...
 */
class ImageToGreyParser : public Vertica::UDParser {
private:
    void insertImage(const char* start, const char* end){
		...
		for (unsigned int y = 0; y < img->rows(); y++){
			for (unsigned int x = 0; x < img->columns(); x++){
				...
				writer->setInt(1, x);
				writer->setInt(2, y);
				writer->setInt(3, static_cast<int>(((255*rgb->red() + 255*rgb->green() + 255*rgb->blue())/3)+0.5));
				writer->next();
...
class ImageToGreyParserFactory : public Vertica::ParserFactory {
public:
    ...
    virtual Vertica::UDParser* prepare(Vertica::ServerInterface &srvInterface,
        ...
        return Vertica::vt_createFuncObject<ImageToGreyParser>(srvInterface.allocator);
    }
...
RegisterFactory(ImageToGreyParserFactory);
